...
public interface CursoRepository extends JpaRepository<CursoEntity, Integer> {
    Iterable<CursoEntity> findCursoByAsignatura_Nombre(String nombre);

    @Query("SELECT c FROM CursoEntity c " +
            "LEFT JOIN FETCH c.franjas fh " +
            "LEFT JOIN FETCH fh.espacioFisico ef " +
            "WHERE c.cursoId = :cursoId")
    Iterable<CursoEntity> findCursoCompletoConFranjasYEspacios(@Param("cursoId") Integer cursoId);
